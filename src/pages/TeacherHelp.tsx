...
export default function TeacherHelp() {
  const steps = [
    ...
    {
      title: "מה עושים אם אין token או שהסנכרון האוטומטי חסום?",
      content: "אין ממציאים נתונים. משתמשים במסלול הקבצים: מורידים את הדוח הרלוונטי ממודל (ציונים, לוגים, השלמות, משתתפים) ומעלים דרך 'ייבוא נתונים'. המערכת מזהה את סוג הדוח לפי הכותרות ומייבאת רק אחרי אימות עמודות. אם אין קובץ תקין וגם אין API — הדוח יסומן 'חסום / נדרש מקור אמת' ולא יוצג."
    },
    {
      title: "האם 'ניהול למידה' של משהח מופיע כאן?",
      content: "לא בשלב זה. לוח 'ניהול למידה' של משרד החינוך (KPIs ו-9 גרפים) קיים בתוך מודל עצמו, אך התוסף לא זמין ב-Web Services הסטנדרטי. המורה משתמש בו ישירות במודל, ואם משהח יחשוף API בעתיד — נשקף אותו גם כאן."
    }
  ];
