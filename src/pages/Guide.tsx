...
const TOPICS = [
  ...
  {
    id: 'moe-analytics',
    title: 'ניהול למידה — תוסף משהח',
    icon: BarChart3,
    color: 'bg-fuchsia-500',
    short: 'לוח מחוונים אנליטי של משרד החינוך בתוך Moodle עצמו',
    content: `
      <p class="mb-4">"ניהול למידה" הוא לוח מחוונים אנליטי ייחודי של משרד החינוך, שקיים בתוך מרחב ה-Moodle שלך. הוא מציג בעיצוב גרפי את נתוני הלמידה בכיתה — ישירות במודל, לא ב-Teacher Hub.</p>

      <p class="mb-4"><strong>מה יש בלוח:</strong></p>
      <ul class="list-disc pr-5 space-y-1 mb-4">
        <li>ממוצע ציונים במרחב</li>
        <li>אחוז השלמה כולל</li>
        <li>מספר תלמידים בקורס</li>
        <li>מספר תלמידים שלא נכנסו כלל</li>
        <li>9 גרפים: יחידות ותלמידים עם הממוצע/ההשלמה הגבוהים והנמוכים, ופעילויות עם ציון מתחת לסף</li>
      </ul>

      <p class="mb-4">לכל גרף בלוח יש כפתור הורדה: SVG או PNG לתמונה, ו-CSV לנתונים הגולמיים.</p>

      <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl mb-4">
        <p class="text-sm text-amber-700"><strong>שים לב:</strong> Teacher Hub לא שולף עדיין נתונים מלוח "ניהול למידה" — התוסף לא זמין ב-Web Services הסטנדרטי של Moodle. כרגע זו יכולת של משהח שהמורה משתמש בה ישירות במרחב. אם ייחשף API עתידי, נשקף אותה גם ב-Teacher Hub.</p>
      </div>
    `,
    screenshot: '<!-- תמונת מסך: לוח ניהול למידה עם 4 כרטיסי KPI וגרפים -->',
    steps: [
      'ב-Moodle — היכנס למרחב הקורס',
      'בסרגל הקורס לחץ על "דוחות"',
      'בחר "ניהול למידה" מרשימת הדוחות',
      'לוח המחוונים ייטען עם KPIs וגרפים',
      'לחץ על תפריט ההורדה של כל גרף כדי לייצא SVG/PNG/CSV',
    ],
    tips: [
      'הלוח מתעדכן אוטומטית לפי נתוני Moodle העדכניים',
      'אפשר להוריד גרפים כתמונות להצגה בישיבות צוות',
      'אם הלוח לא מופיע — לפנות למנהל המערכת של המוסד',
    ],
  },
  {
    id: 'export',
