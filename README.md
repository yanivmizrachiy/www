...
ניקיון הריפו אינו תלוי יותר בבדיקה ידנית בלבד.

הבדיקה הקנונית היא:

`node scripts/checks/repo-doctor.cjs`

GitHub Actions מריץ אותה דרך:

`.github/workflows/repo-hygiene.yml`

ה־workflow רץ בכל `push`, בכל Pull Request ל־`main`, וגם ידנית בעת הצורך. הוא נכשל כאשר חוזרים לריפו, בין היתר:

- paths שהוצאו משימוש כמו `archive/`, `downloads/`, `WORK_ORDERS/` ו־AI/audit dumps ישנים;
- build/generated junk כמו `dist/`, `coverage/`, logs, backups ו־temp files;
- raw Moodle exports או קבצי נתונים פרטיים;
- secret patterns מוכרים;
- חסר באחד ממסמכי המקור הקנוניים;
- הרחבת August שמקבלת host scope רחב יותר מה־Moodle המאושר.

`npm run doctor` משתמש באותה בדיקה ולכן גם ה־CI הראשי אוכף את אותה מדיניות.

אין לעקוף את השער על ידי יצירת מקור אמת חדש או שינוי שמות לקבצי זבל; אם פריט חדש באמת נחוץ, יש לעדכן במפורש את מדיניות הריפו ואת ה־doctor באותו שינוי.
...
`yanivcar/` אינו מוצר Moodle, אך נשאר זמנית משום שקיים בו endpoint עדכון שעלול להיות נצרך על ידי לקוח חיצוני. אין למחוק אותו לפני אימות שאין צרכן פעיל או לאחר העברה מאומתת.
