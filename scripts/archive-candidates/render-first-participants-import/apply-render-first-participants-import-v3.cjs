...
function replaceBetween(text, startNeedle, endNeedle, replacement, label) {
  ...
  if (start < 0 || end < 0 || end <= start) throw new Error('Cannot replace section: ' + label);
  return text.slice(0, start) + replacement + '\n\n' + text.slice(end);
}

let server = read('src/server.js');
mustInclude(server, 'function sessionFromRequest(req)', 'sessionFromRequest');
mustInclude(server, 'function buildGradesCsv()', 'buildGradesCsv');
mustInclude(server, 'app.get("/api/imports/overview"', 'imports overview route');

const helpers = String.raw`function importSessionFromRequest(req) {
  const body = req.body || {};
...
function normalizeImportedStudent(row, session) {
  ...
  const email = pickImportValue(row, ["כתובת דואל", "כתובת דואל", "דואל", "דואר אלקטרוני", "Email address", "Email"]);
  ...
  const spaceId = session && session.spaceId ? session.spaceId : "unknown-space";
...
if (!server.includes('function importSessionFromRequest(req)')) {
  server = server.replace('function buildGradesCsv()', helpers + '\nfunction buildGradesCsv()');
}

const routes = String.raw`app.post("/api/import", (req, res) => {
...
if (!server.includes('app.post("/api/import"')) {
  server = server.replace('app.get("/api/imports/overview"', routes + 'app.get("/api/imports/overview"');
}

write('src/server.js', server);

let imports = read('src/hooks/useImports.tsx');
mustInclude(imports, 'export function useImportedStudents()', 'useImportedStudents');
mustInclude(imports, 'export async function postImport', 'postImport');

const newUseImportedStudents = String.raw`export function useImportedStudents() {
...
imports = replaceBetween(imports, 'export function useImportedStudents()', 'export interface GradesMatrix', newUseImportedStudents, 'useImportedStudents');

const newPostImport = String.raw`export async function postImport(body: {
...
imports = replaceBetween(imports, 'export async function postImport', 'export interface CourseChapter', newPostImport, 'postImport');

write('src/hooks/useImports.tsx', imports);
console.log('PATCH_RENDER_FIRST_PARTICIPANTS_IMPORT_V3_OK');
